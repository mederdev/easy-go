...
function resetForm(): void {
  form.model = 'KIA Carnival';
  form.plate = '';
  form.driverId = '';
  form.seats = 11;
  form.status = 'AVAILABLE';
  form.locationCity = cities[0];
}
...
function openEdit(c: Car): void {
  ...
  form.model = c.model;
  form.plate = c.plate;
  form.driverId = c.driverId ?? '';
  form.seats = c.seats;
  form.status = c.status;
  form.locationCity = c.locationCity ?? cities[0];
  formError.value = null;
  modalOpen.value = true;
}
...
async function save(): Promise<void> {
  ...
  if (!form.model.trim() || !form.plate.trim()) {
    formError.value = 'Укажите модель и госномер.';
    return;
  }
  saving.value = true;
  try {
